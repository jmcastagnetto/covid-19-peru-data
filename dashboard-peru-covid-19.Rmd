...
covid_regions_pe <- read_csv(
  ...
  distinct() %>% 
  mutate(
    confirmed_per_million = round(confirmed * 1e6 / pop2020, 1)
  )
...
df2 <- covid_pe %>% 
  ...
  mutate(
    ...
    new_death = ifelse(new_death < 0, 0, new_death), # fix outliers from the start of the epidemic
    new_disc = negative_tests - lag(negative_tests),
    new_tot = new_conf + new_disc,
    new_tot = ifelse(new_tot < 0, 0, new_tot) # fix outliers from the start of the epidemic
  ) %>%
...
pt1 <- ggplot(df2, aes(x = date, y = new_conf)) +
  ...
  labs(
    ...
    caption = "Curva: aproximación GAM [y ~ s(x, bs = 'cs')]"
    #subtitle = minsa,
    #caption = paste0(last_updated, " // ", jmc)
  ) +
  cur_theme
...
pt3 <- ggplot(df2, aes(x = date, y = new_death)) +
  ...
  labs(
    ...
    caption = "Curva: aproximación GAM [y ~ s(x, bs = 'cs')] // Truncado en 250"
    #subtitle = minsa,
...
ggplot(
  ...
  ggforce::geom_mark_ellipse(
    data = subset_df %>% filter(outofbounds),
    aes(group = outofbounds),
    color = "peru"
  ) +
  scale_color_manual(
    values = c("purple", "peru")
  ) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    x = "",
...
datatable(
  ...
  colnames = c("País", 
               ...
               "Positivos por moleculares y serológicas",
...
[`r fontawesome::fa("twitter")`](https://twitter.com/jmcastagnetto)
[`r fontawesome::fa("github")`](https://github.com/jmcastagnetto')
[`r fontawesome::fa("home")`](https://castagnetto.site')
