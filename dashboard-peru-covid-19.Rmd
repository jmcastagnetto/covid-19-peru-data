...
covid_regions_pe <- read_csv(
  ...
  select(region, cod_dep_inei, cod_dep_reniec, iso_3166_2_code, 
         ...
         pcr_positivo, prueba_rapida_positivo, pob_2017) %>% 
  distinct() %>% 
  mutate(
...
pe_plot <- ggplot(covid_pe,
  ...
  geom_point(aes(tooltip = confirmed)) +
  geom_line() +
  geom_vline(xintercept = as.Date("2020-03-16"), color = "blue", size = 2, alpha = .5) +
  annotate(geom = "text", x = as.Date("2020-03-16"), y = 300, hjust = 1.1,
...
map1_max <- ((max(map_df$confirmed) %/% 1000) + 1) * 1000
...
map2_max <- ((max(map_df2$confirmed_per_million) %/% 100) + 1) * 100
...
tests_reg <- covid_regions_pe %>% 
  filter(!is.na(pcr_positivo)) %>% 
  select(region, date, pcr_positivo, prueba_rapida_positivo) %>% 
  pivot_longer(
    cols = c(pcr_positivo, prueba_rapida_positivo),
...
ggplot(
  ...
  scale_fill_manual(
    ...
    labels = c("PCR (molecular)", "Serológica (\"rápida\")")
  ) +
  labs(
    y = "",
    x = "",
