...
datatable(
  ...
  formatStyle(
    "discarded_cases", 
    color = "black",
    background = styleColorBar(sit_pe$discarded_cases, 'cyan'),
    backgroundSize = '90% 90%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  ) %>%
...
future0 <- predict(model0, newdata = data.frame(t = ts_seq))
df0_2 <- data.frame(t = ts_seq, confirmed = future0, date = date_seq)

df$preds <- predict(model1)
future <- predict(model1, newdata = data.frame(t = ts_seq))
df2 <- data.frame(t = ts_seq, confirmed = future, date = date_seq)

future_prev <- predict(model_prev, newdata = data.frame(t = ts_seq))
df_prev2 <- data.frame(t = ts_seq, confirmed = future_prev, date = date_seq)
