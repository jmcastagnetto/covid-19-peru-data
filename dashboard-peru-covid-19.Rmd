...
covid_regions_pe <- read_csv(
  ...
  filter(!is.na(region)) %>% 
  group_by(region, cod_dep_inei, cod_dep_reniec,
           iso_3166_2_code, date, pob_2017) %>% 
  summarise(
    confirmed = sum(confirmed, na.rm = TRUE),
    deaths = sum(deaths, na.rm = TRUE),
    negative_cases = sum(negative_cases, na.rm = TRUE),
    pcr_positivo = sum(pcr_positivo, na.rm = TRUE),
    prueba_rapida_positivo = sum(prueba_rapida_positivo, na.rm = TRUE),
    pcr_prapida_positivo = sum(pcr_prapida_positivo, na.rm = TRUE)
  ) %>%
  distinct() %>% 
  mutate(
