...
### Casos analizados clínicamente: confirmados y rechazados
...
df <- covid_pe %>% 
  ...
  pivot_longer(
    ...
    names_to = "Casos analizados clínicamente",
    values_to = "cases"
  ) %>% 
  select(
    date, `Casos analizados clínicamente`, cases
  )

bar_plot <- ggplot(df, 
       aes(x = date, y = cases, fill = `Casos analizados clínicamente`)) +
  geom_col() +
  theme_minimal() +
  labs(
    y = "Número de casos",
    x = "",
    title = "COVID-19: Casos totales analizados clínicamente en el Perú",
    subtitle = minsa,
...
pt2 <- ggplot(df2, aes(x = date, y = new_tot)) +
  ...
  theme_minimal() +
  labs(
    y = "Casos analizados clínicamente",
    x = "",
    title = "COVID-19: Nuevos casos analizados clínicamente totales (Perú)",
    subtitle = minsa,
    caption = paste0(last_updated, "\n", jmc)
  ) +
  cur_theme
...
### Nuevos casos analizados clínicamente totales
